...
head:
    - - meta
      - property: 'og:title'
        content: Elysia 1.0 - 陨落者的哀歌

    - - meta
      - name: 'description'
        content: 介绍蔗糖，一个更好的静态代码分析引擎，启动时间提升高达 14 倍，移除每实例 40 个路由的限制，类型推断速度提升约 3.8 倍，更新条约 2，钩子类型（重大更改），以及严格类型检查的行内错误。
...
Elysia 1.0 是经过 1.8 年开发后的首个稳定版本。

自从开始，我们一直在期待一个专注于开发者体验、开发速度，以及如何为人类而不是机器编写代码的框架。

我们在各种情况下测试了 Elysia，模拟了中等到大型的项目，并将代码交付给客户，这是我们感到足够自信可以发布的第一个版本。

Elysia 1.0 带来了重大改进，并包含了一个必要的重大变更。
...
::: tip
请记住，Elysia JS 是由志愿者维护的开源库，与米哈游或霍格沃兹没有关联。但我们确实是崩坏系列的超级粉丝，对吗？:::
:::

## 蔗糖
Elysia 优化了性能，在各种基准测试中表现出色，其中一个主要因素要归功于 Bun，以及我们的自定义即时静态代码分析。

如果你还不知道，Elysia 有一个内置的 “编译器”，它读取你的代码并生成一种优化处理函数的方法。
...
## 移除约 40 条路由/实例限制
以前，由于 Elysia 0.1 的限制，你只能堆叠约 40 条路由/1 个 Elysia 实例。
...
然而，从 Elysia 1.0 开始，我们在优化类型性能，特别是尾调用优化和变体方面克服了这个限制。
...
阻止我们感觉 Elysia 还没有准备好生产的是这个限制。

## 类型推断速度提升
多亏了我们在优化方面所做的努力，我们在大多数 Elysia 服务器中测量了**约 82%**的提升。
...
总的来说，Elysia 和 Eden Treaty 一起工作，速度可以**提升约 3.9 倍**。
...
## 更新 Treaty 2
我们向你征求对 Eden Treaty 的意见，你喜欢什么，有什么可以改进的地方。你提出了关于 Treaty 设计的一些缺陷，并提出了几种改进措施。

这就是为什么今天，我们介绍 Eden Treaty 2，一个更加符合人体工程学的设计。

尽管我们不喜欢重大变更，但 Treaty 2 是 Treaty 1 的继承者。
...
因此，我们可以在不发送 FETCH 请求的情况下，用 Eden Treaty 2 来编写单元测试，提供自动完成和类型安全，而不是启动一个模拟服务器。
```typescript
// test/index.test.ts
...
两个之间的区别是 **Treaty 2 是 Treaty 1 的继承者。**

我们并不打算引入任何条约 1 的破坏性变更，也不强迫你更新到条约 2。

你可以继续使用条约 1 而不需要更新到条约 2，我们将以维护模式维护它。
...
Elysia 核心本身部分兼容 WinterCG，但是不是所有的官方插件在 WinterCG 上工作，有一些 Bun 特定的功能，我们想要修复这一点。
...
Elysia 是一个开源开发者的作品，没有任何公司支持。

我们需要做一些工作来维持生计，并在业余时间构建 Elysia。

在一个点上，我选择不立即寻找工作，只是为了在几个月内专注于 Elysia。

我们愿意花费我们的时间来不断改进 Elysia，并且你可以通过 [GitHub 赞助商](https://github.com/sponsors/SaltyAom)帮助我们，以减少我们支持自己的工作量，并拥有更多的时间来工作在 Elysia。
