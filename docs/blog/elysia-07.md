...
类型是 Elysia 最重要的方面之一，因为它允许我们做许多惊人的事情，比如统一类型，同步你的业务逻辑，类型，文档和前端。

我们希望你能够体验 Elysia 的卓越体验，专注于你的业务逻辑部分，让 Elysia 处理剩余的事情，无论是类型推理与统一类型，还是 Eden 连接器，用于与后端同步类型。
...
我们希望这个改进将帮助你实现更快的类型推理，比如更快的自动完成和 IDE 加载时间，帮助你的开发变得更加快速和流畅。

## Trace

性能是 Elysia 的另一个重要方面。

我们不希望 Elysia 只是为了基准测试而快，我们希望你有一个真实的、快速的服务器，而不是仅仅为了基准测试。

有许多因素可能会减慢你的应用程序，很难识别一个，这就是我们引入**追踪**的原因。
...
这不仅限于 `beforeHandle`，事件也可以追踪 `handler` 本身。命名约定是你已经熟悉的生命周期事件的名称。

这个 API 让我们轻松地审计 Elysia 服务器的性能瓶颈，并与你选择的报告工具集成。

默认情况下，追踪使用 AoT 编译和动态代码注入来条件性地报告，这意味着如果你实际上没有使用，就不会有任何性能影响。
...
当你需要使用 cookie 时，只需提取名称来获取/设置其值：
```typescript
app.get('/', ({ cookie: { name } }) => {
    // Get
    name.value
...
Elysia 为你自动编码和解码 cookie 值，因此如果你需要将 JSON 存储在 cookie 中，例如解码的 JWT 值，或者只是确保值是数字字符串，你可以这样做而不费吹灰之力。
...
要处理 Cookie 签名，你只需提供 `secert` 和 `sign` 属性：
```typescript
new Elysia({
    cookie: {
...
Elysia 自动处理 Cookie 的密钥轮换，因此如果你需要迁移到新的 cookie 密钥，你可以只追加密钥，Elysia 将使用第一个值来签名新的 cookie，同时尝试使用剩余的密钥解签 cookie，如果匹配。
```typescript
new Elysia({
    cookie: {
...
当你必须使用 **multipart/formdata** 来处理文件上传，但它不支持对象时，这非常有用。现在，你可以使用 `t.ObjectString()` 来告诉 Elysia 该字段是一个字符串化的 JSON，因此 Elysia 可以自动解码它。
```typescript
new Elysia({
    cookie: {
        secret: 'Fischl von Luftschloss Narfidort'
...
当你需要处理一个具有重复名称的插件时，这很有用，它允许你重新映射插件的名称：
```typescript
new Elysia()
    .use(
        plugin
...
我们希望映射和后缀将提供一个强大的 API，帮助你处理多个复杂的插件。
...
当你输入时，应该会有 IDE 的自动完成弹出，无论是 NeoVim 还是 VSCode，因为这是 TypeScript 的内置功能。

![显示自动完成文本基状态码的示例](/blog/elysia-07/teapot-autocompletion.webp)

这是一个小的 ergonomic 特性，帮助你开发服务器，无需在 IDE 和 MDN 之间切换，搜索正确的状态码。
