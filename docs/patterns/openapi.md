...
<summary>类型生成遇到问题？</summary>

### 注意：显式类型
OpenAPI 类型生成在使用隐式类型时效果最佳。

有时，显式类型可能导致生成器无法正确解析。

此时，你可以使用 `Prettify` 来内联类型：
```ts
import { Elysia, t } from 'elysia'
...
### 注意：根路径
由于根目录难以准确推测，建议明确提供项目根目录路径，以保证生成器正常运行，特别是当使用 monorepo 时。
...
## Response headers
我们可以通过用 `withHeader` 包裹 schema 来添加响应头：
...
然后使用端点配置部分的 detail 属性将该端点分配到某个分组
