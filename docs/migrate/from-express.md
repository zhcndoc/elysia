...
Express 使用专门的 `express.Router()` 来声明子路由，而 Elysia 将每个实例视为可插拔组合的组件。
...
> Elysia 使用 TypeBox 进行验证并自动转换类型，同时支持使用相同语法的各种验证库如 Zod 和 Valibot。
...
> Express 需要 `multer` 处理文件上传并手动验证 MIME 类型
