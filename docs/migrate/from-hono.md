...
**Elysia**
- **最初为原生 Bun 构建**，充分利用了 Bun 的大部分功能
- 支持使用 Web 标准的多种运行时，包括 **Node.js** 和 **Cloudflare Workers**
- **更好的性能**。通过 JIT 面向长时间运行的服务器。
- **更好的 OpenAPI 支持**，体验流畅，尤其是通过 [OpenAPI 类型生成](/patterns/openapi#openapi-from-types)
- 偏向基于事件的生命周期方法，更好地控制请求管道
- 全面的类型安全，包括中间件和错误处理
- 在中间件、封装和插件风格上有点类似 Fastify
...
两者都使用单个 `Context` 参数来处理请求和响应，直接返回响应。
...
> Elysia 使用单一 `context` 并直接返回响应

</template>

</Compare>

虽然 Hono 使用 `c.text` 和 `c.json` 包装响应，Elysia 会自动将返回值映射为响应。

在风格指南上有细微差异，Elysia 推荐使用方法链和对象解构。

Hono 的端口分配依赖运行时和适配器，而 Elysia 使用单一的 `listen` 方法启动服务器。

## 处理程序

Hono 通过函数手动解析查询、头和主体，而 Elysia 自动解析属性。
...
> Elysia 使用静态代码分析自动分析需要解析的内容

</template>

</Compare>

Elysia 使用 **静态代码分析** 来确定所需解析的内容，仅解析必要的属性。

这对性能和类型安全非常有利。

## 子路由

两者都支持作为路由器继承另一个实例，但 Elysia 将每个实例视为可用作子路由器的组件。
...
> Elysia 使用可选的前缀构造函数参数来定义路由前缀

</template>

</Compare>

虽然 Hono 需要前缀来区分子路由器，Elysia 不强制需要。

## 验证
虽然 Hono 通过外部包支持各种验证器，Elysia 内置了基于 **TypeBox** 的验证，且开箱即用支持标准模式（Standard Schema），允许您直接使用喜欢的库如 Zod、Valibot、ArkType、Effect Schema 等，无需额外库。Elysia 还提供与 OpenAPI 的无缝集成和类型推断。
...
> Elysia 使用 TypeBox 进行验证并自动转换类型。同时也支持 Zod、Valibot 等各种验证库，且语法一致。
...
> Elysia 声明式处理文件和 MIME 类型验证

</template>

</Compare>

由于 Web 标准 API 不验证 MIME 类型，因此信任客户端提供的 `content-type` 可能存在安全风险，故 Hono 依赖外部库，而 Elysia 使用 `file-type` 自动验证 MIME 类型。

## 中间件

Hono 中间件采用类似 Express 的单队列顺序执行，而 Elysia 使用 **基于事件** 的生命周期，为你提供更细粒度的控制。

Elysia 的生命周期事件如图所示。
![Elysia 生命周期图](/assets/lifecycle-chart.svg)
> 单击图片放大

虽然 Hono 的请求管道是单一流程，Elysia 可拦截请求管道中的每个事件。
...
> Elysia 为请求管道中的每个节点使用特定的事件拦截器

</template>

</Compare>

虽然 Hono 通过 `next` 函数调用下一个中间件，Elysia 中无 `next` 函数。

## 严格的类型安全
Elysia 设计为提供严格的类型安全。

例如，你可以使用 [derive](/essential/life-cycle.html#derive) 和 [resolve](/essential/life-cycle.html#resolve) 以 **类型安全** 的方式自定义上下文，Hono 则不能。
...
虽然 Hono 可以使用 `declare module` 扩展 `ContextVariableMap` 接口，但它是全局可用的，因此不具备类型安全，且无法保证该属性在所有请求处理程序中都可用。
...
> Hono 使用回调返回 `createMiddleware` 来创建可重用的中间件，但类型安全性不足
...
class CustomError extends Error {
	...
	toResponse() {
		return {
			message: "如果你看到这个，说明我们的开发忘记处理此错误了",
			error: this
		}
	}
}
...
虽然 Hono 使用类中间件的方式处理错误，Elysia 提供了：

1. 全局及路由特定的错误处理器
2. HTTP 状态码映射的简写，以及通过 `toResponse` 方法映射错误为响应
3. 为每个错误提供自定义错误码

错误码对日志和调试非常有用，也有助于区分继承同一类的不同错误类型。

Elysia 在这方面具备类型安全，而 Hono 则没有。
...
但 Elysia 可以通过声明作用域插件显式指出插件的副作用范围，而 Hono 总是进行封装。
...
2. **scoped** - 作用域限制在父实例，但不超出
3. **global** - 影响所有实例

---

因为 Hono 不提供作用域机制，我们需要：

1. 为每个钩子创建函数并手动附加
2. 使用高阶函数，并应用到需要副作用的实例
...
在这种场景下，Elysia 提供了插件去重机制防止副作用重复。
...
通过使用唯一的 `name`，Elysia 只会应用该插件一次，避免重复的副作用。
...
Hono 有单独的函数来描述路由规范、验证，并且需要一些额外配置才能正确运行。

Elysia 使用你提供的模式来生成 OpenAPI 规范，自动验证请求/响应，并从**单一真实来源**推断类型。

Elysia 还会将注册的模式附加到 OpenAPI 规范中，允许你在 Swagger 或 Scalar UI 中的专用部分引用该模型，而 Hono 则把模式内联到路由中。
...
Elysia 设计为符合人体工学且对开发者友好，专注于**健全的类型安全**，并且性能优于 Hono。
...
<Deck>
	<Card title="从 Express 迁移" href="/migrate/from-express">
		Express 和 Elysia 的对比
	</Card>
    <Card title="从 Fastify 迁移" href="/migrate/from-fastify">
  		Fastify 和 Elysia 的对比
    </Card>
    <Card title="从 tRPC 迁移" href="/migrate/from-trpc">
  		tRPC 和 Elysia 的对比
    </Card>
</Deck>
